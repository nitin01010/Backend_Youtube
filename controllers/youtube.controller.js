...
async function handleCreateChannel(req, res) {
  try {
    ...
    if (!id || !channelName) {
      return res.status(400).json({ message: "User ID and channel name are required." });
    }
    ...
    if (user.channels.length >= 1) {
      return res.status(400).json({ message: "User already has a channel. Only one channel is allowed." });
    }

    // Create and add the first channel
    user.channels.push({ channelName, videos: [] });
    await user.save();

    res.status(200).json({ message: "Channel created successfully.", user });
  } catch (error) {
